"use client";

import React, { useEffect, useState } from "react";
import { useRouter } from "next/navigation";
import { db } from "@/library/firebase";
import { doc, getDoc } from "firebase/firestore";

interface SuccessPageProps {
  params: { id: string }; // Assuming you get params from Next.js dynamic routes
}

export default function SuccessPage({ params }: SuccessPageProps) {
  const [userData, setUserData] = useState<any | null>(null); // For storing fetched user data
  const [loading, setLoading] = useState(true); // To show loading state while data is being fetched
  const router = useRouter();

  useEffect(() => {
    const fetchUserData = async () => {
      try {
        const userRef = doc(db, "users", params.id);
        const userDoc = await getDoc(userRef);

        if (userDoc.exists()) {
          setUserData(userDoc.data());
        } else {
          console.error("No such user!");
        }
      } catch (error) {
        console.error("Error fetching user data:", error);
      } finally {
        setLoading(false); // Stop loading after data fetching completes
      }
    };

    fetchUserData();
  }, [params.id]);

  const handleBackToLogin = () => {
    router.push("/login"); // Navigate back to the login page
  };

  if (loading) {
    return <p>Loading...</p>; // Display loading state while data is being fetched
  }

  return (
    <div className="p-6 max-w-xl mx-auto h-screen flex flex-col justify-center items-center">
      <h1 className="text-2xl font-bold mb-4 text-center">
        Congratulations, Please Login to upload the documents!
      </h1>


      {/* Button to go back to the login page */}
      <div className="flex gap-4 justify-center">
        <button
          onClick={handleBackToLogin}
          className="px-4 py-2 bg-gray-600 text-white rounded hover:bg-gray-700 transition"
        >
          Back to Login
        </button>
      </div>
    </div>
  );
}
